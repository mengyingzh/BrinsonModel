...
def draw_graph(result1, result2, result3):
    ...
    # draw allocation
    pd_df = pd.DataFrame(list(result1.items()))
    ...
    # draw selection
    pd_df = pd.DataFrame(list(result2.items()))
    ...
    # draw interaction
    pd_df = pd.DataFrame(list(result3.items()))
    ...
    plt.tight_layout()
    plt.savefig("brinson_model_interaction.png", dpi=120)


if __name__ == '__main__':
    data = xlrd.open_workbook('SL48xx_xxx私募证券投资基金_资产估值表_20171208.xls')  #privacy preserved
    sheet = data.sheets()[0]
    nrows = sheet.nrows
    ncols = sheet.ncols
    ...
    for industry_key in fund_yield_dic.keys():
        allocation_result[industry_key] = round((fund_weight_dic.get(industry_key)
                                                 - bench_weight_dic.get(industry_key))
                                                * bench_yield_dic.get(industry_key), 6)

    selection_result = {}
    for industry_key in fund_yield_dic.keys():
        selection_result[industry_key] = round((fund_yield_dic[industry_key]
                                                - bench_yield_dic[industry_key])
                                               * bench_weight_dic[industry_key], 6)

    interaction_result = {}
    for industry_key in fund_yield_dic.keys():
        interaction_result[industry_key] = round(fund_yield_dic[industry_key]
                                                 * fund_weight_dic[industry_key]
                                                 - bench_yield_dic[industry_key]
                                                 * bench_weight_dic[industry_key], 6)
